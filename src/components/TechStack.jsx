...
const TechStack = () => {
  ...
  const tooltipBorder = theme ? 'border border-black/20' : 'border border-gray-200/50';
  const tooltipArrow = theme ? 'border-t-black/95' : 'border-t-white/95';
  const cardBg = theme ? 'bg-white shadow-md border-gray-200/50' : 'bg-gray-900 shadow-lg border-gray-700/50';
  const cardHoverBg = theme ? 'hover:bg-gray-50' : 'hover:bg-gray-800';
  ...
  return (
    <section className="px-6 mt-10.5 mb-0 ">
      <motion.h2 
        ...
        className={`mb-6 text-start text-[18px] font-bold ${theme ? 'text-black' : 'text-white'}`}
      >
        Tools that I have used
      </motion.h2>
      <AnimatePresence mode='wait'>
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-7 xl:grid-cols-11 gap-4 max-w-5xl mx-auto relative">
