...
const Contact = () => {
  ...
  // Inline SVG icons so you don't need any external npm package
  const links = [
    { 
      id: '01', 
      label: 'Email', 
          ...
          {links.map((link) => {
            return (
              <a
                key={link.id}
                href={link.href}
                // Removes target="_blank" only for mailto links so it won't refresh the UI
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className={`w-10 h-10 flex items-center justify-center rounded-lg border group-hover:text-emerald-500 group-hover:border-emerald-500/20 transition-colors duration-300 ${iconBoxBg}`}>
                    {/* Render the inline SVG function passing standard sizes */}
                    {link.icon("w-4 h-4")}
                  </div>
